...
O frontend executa typecheck, testes de comportamento e build de produção no CI. Os fluxos cobertos incluem upload, polling terminal, analytics, estados de erro, paginação por cursor e virtualização das coleções. O gerador determinístico é validado separadamente. O benchmark oficial não roda em runner compartilhado porque seus resultados precisam de recursos de hardware controlados.
